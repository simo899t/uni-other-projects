...
if __name__ == '__main__':
    ...
    mult_numpy(M, v)
    mult_mat(Mat(M.tolist()), Vec(v.tolist()))

    mult_numpy(u, M)
    mult_mat(Vec(u.tolist()), Mat(M.tolist()))

    mult_numpy(M, N)
    mult_mat(Mat(M.tolist()), Mat(N.tolist()))
    ...
    M_s = sps2sparseMat(M)
    N_s = sps2sparseMat(N)
    M_m = Mat(M.toarray().tolist())
    N_m = Mat(N.toarray().tolist())
